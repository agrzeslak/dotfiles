...
- **`<class>` token** — single token, makes the ledger searchable; tag a candidate with the root primitive it came from (see `review-primitives.md`). One of: `trace` (producer-realization / returned-value-discarded / dead-state / serialization / semantic-mismatch / dispatch-chain), `sibling` (behavior / guard / authority / error-branch / backward-compat / fix-site parity), `negative-space` (old-inputs / predicate-permissive / cache-key / persistent-state / migration), `async` (invalidation / mount-time / teardown), `narrative` (doc-vs-impl / new-vs-reused / adr-as-policy / claim-true-carve-out), `test-fixture` (cross-cutting), `other`.
