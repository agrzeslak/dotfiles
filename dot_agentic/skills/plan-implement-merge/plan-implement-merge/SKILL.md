   ...
   **TDD for fixes — when appropriate:** Before implementing a fix, decide whether a failing
   ...
   Report back:
     - Counts of findings by severity, per reviewer that ran (custom-review always; codex only
       if the gate ran it; the repo-local gate-check reviewer when the repo defines one —
       multi-review runs it automatically) — counted from the *pre-fix* review output, before
       any fixes.
